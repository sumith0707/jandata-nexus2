...
def validate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        ...
        method = str(row.get("entity_resolution_method", "")).lower()

        # Only an unresolved district is a hard entity error.
        if entity.startswith("UNRESOLVED:"):
        ...
        # Fuzzy matches are accepted; only genuinely weak/unresolved rows
        # are hard-flagged.
        if confidence < 0.35 and method in {
            "unresolved", "empty_input", "unresolved_short_input"
        }:
        ...
        if not _is_finite(value):
            ...
            if not (lo <= numeric <= hi):
                reasons.append(f"out_of_range({numeric:g})")

        if reasons:
            df.at[idx, "validation_flag"] = True
            df.at[idx, "validation_reason"] = ";".join(reasons)

    # Duplicates are useful information, but they are NOT automatically
    # invalid. They can come from repeated/overlapping government tables.
    if {"entity_name", "year", "indicator"}.issubset(df.columns):
        dup_mask = df.duplicated(
            subset=["entity_name", "year", "indicator"], keep=False
        )
        for idx in df.index[dup_mask]:
            existing = str(df.at[idx, "validation_reason"] or "")
            if "duplicate_entry" not in existing:
                df.at[idx, "validation_reason"] = (
                    f"{existing};duplicate_entry" if existing else "duplicate_entry"
                )
